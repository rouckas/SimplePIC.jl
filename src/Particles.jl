...
Base.zero(::Union{Type{Particle1d3vE}, Particle1d3vE}) = Particle1d3vE(zero(SVector{1, Float64}), zero(SVector{3, Float64}), zero(SVector{3, Float64}), zero(SVector{3, Float64}), 0., 0.)
...
ParticleEnsemble(name, m, q, n, ParticleType) = ParticleEnsemble{ParticleType}(
        name, m, q,

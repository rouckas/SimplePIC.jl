...
export epsilon_0, e
export PIC, Diagnostic, RhoProbe, EnergyProbe, NxProbe, NvxProbe, EProbe, PSxProbe, UProbe, TProbe, sample!
...
function PIC(particles::Vector{ParticleEnsemble{ParticleType}}, interactions::Vector{Interactions{ParticleEnsemble{ParticleType}}},
    ...
    PIC{1, ParticleType, Cartesian1D, BCType, vdim}(particles,
        ...
        0.,
        epsilon_0, 0.0, 0.0, zeros(SVector{vdim, Float64}, geo.nx), zeros(SVector{vdim, Float64}, geo.nx), zeros(SVector{vdim, Float64}, geo.nx), zeros(SVector{vdim, Float64}, geo.nx))
end
...
function advance(pic::PIC, dt::Float64, circuit::Circuit, t::Float64)
    for (p, inter) in zip(pic.particles, pic.interactions)
        advance(p, inter, dt)
        particle_bc(p, pic, pic.xmax, pic.BC)
        advance_external(pic, circuit, t, dt)
    end
    #return deltaleft, deltaright
end

function boris_push(pic::PIC, dt::Float64)

end

function init_leapfrog(pic::PIC, dt::Float64, circuit::Circuit, t::Float64)
    sample(pic)
    poisson_solve(pic)
    interpolate(pic)
    for p in pic.particles
